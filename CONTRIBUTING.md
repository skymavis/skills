...
2. Edit `skills/<your-skill>/SKILL.md`: set `name` to equal the folder, and write a
   third-person `description` (≤ 1024 chars) that says **what it does and when to
   use it** — this is the trigger surface, so be specific.
...
## Tests
- Keep the provided `tests/conftest.py` — it puts `scripts/` on `sys.path` so
...
enforced by `gitlint` — a `commit-msg` hook locally and a CI gate on every PR (config
